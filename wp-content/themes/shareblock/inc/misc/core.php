...
if(get_theme_mod('disable_post_related') !=1){
                        ...
                        <h4><?php echo shareblocktxt::shareblock_s_related_articles(); ?></h4>
...
if ( !function_exists( 'shareblock_editor_dynamic_css' ) ){
    function shareblock_editor_dynamic_css() {
        ob_start();
...
function shareblock_review_box($post_id, $class, $echo = true) {    
    ...
    $output .= '<h6 class="jl_sum_title">'.shareblocktxt::shareblock_s_summary().'</h6>';
    $output .= '<p>'.$review_summary.'</p>';    
    $output .= '</div>';
    if($review_pos){
      $output .= '<div class="jl_review_pros">';
      $output .= '<h6>'.shareblocktxt::shareblock_s_the_pros().'</h6>';
      $output .= '<span>'.str_replace(array("\r","\n\n","\n"),array('',"\n","</span>\n<span>"),trim($review_pos,"\n\r")).'</span>';
      $output .= '</div>';
    }
    if($review_neg){
      $output .= '<div class="jl_review_cons">';
      $output .= '<h6>'.shareblocktxt::shareblock_s_the_cons().'</h6>';
...
if ( ! function_exists( 'shareblock_loadnavs' ) ) {
    function shareblock_loadnavs() {
        ...
        if(empty(get_theme_mod('jl_opt_lazy_img'))){
        ...
        if ( $query_data->have_posts() ) {
            ...
            if ( $response['page_current'] == $response['page_max'] ) {
                $response['notice'] = '<div class="' . $notice_class . '"><span class="jl_end_post">' . shareblocktxt::shareblock_s_no_post_to_load() . '</span></div>';
            }
...
if ( ! function_exists( 'shareblock_blocknav_loadmore' ) ) :
    function shareblock_blocknav_loadmore( $query_data = null ) {
        ...
        <div class="jl_lmore_wrap">
            ...
            <a href="#" class="jl-load-link"><span><?php echo shareblocktxt::shareblock_s_load_more(); ?></span></a>
            <span class="jl-load-animation"><span></span></span>
        </div>
        </div>
    <?php
...
add_action( 'woocommerce_before_shop_loop_item_title', 'shareblock_buttons_wrapper_open', 15 );
add_action( 'woocommerce_before_shop_loop_item_title', 'woocommerce_template_loop_add_to_cart', 25 );
add_action( 'woocommerce_before_shop_loop_item_title', 'shareblock_buttons_wrapper_close', 95 );
add_action( 'woocommerce_before_shop_loop_item_title', 'shareblock_loop_product_thumbnail', 10 );
add_action( 'woocommerce_shop_loop_item_title', 'shareblock_loop_product_title', 10 );
add_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_rating', 12 );
add_action( 'woocommerce_checkout_before_customer_details', 'shareblock_checkout_customer_details_before' );
add_action( 'woocommerce_checkout_after_customer_details', 'shareblock_checkout_customer_details_after' );
add_action( 'woocommerce_checkout_after_order_review', 'shareblock_checkout_order_after' );
add_action( 'woocommerce_after_cart', 'shareblock_cart_after', 20 );
add_action( 'woocommerce_after_cart', 'woocommerce_cross_sell_display', 25 );
add_action( 'woocommerce_before_single_product', 'shareblock_single_product_before', 5 );
add_action( 'woocommerce_after_single_product', 'shareblock_single_product_after', 25 );
add_action( 'woocommerce_before_single_product_summary', 'shareblock_single_product_image_before', 1 );
add_action( 'woocommerce_before_single_product_summary', 'shareblock_single_product_summary_before', 100 );
add_action( 'woocommerce_after_single_product_summary', 'shareblock_single_product_summary_after', 1 );
add_action( 'woocommerce_after_main_content', 'woocommerce_get_sidebar', 5 );
add_action( 'woocommerce_before_shop_loop', 'shareblock_wc_before_shop_loop', 5 );
add_action( 'woocommerce_no_products_found', 'shareblock_wc_before_shop_loop', 5 );
add_action( 'woocommerce_after_shop_loop', 'shareblock_wc_after_shop_loop', 99 );
add_action( 'woocommerce_no_products_found', 'shareblock_wc_after_shop_loop', 99 );
add_action( 'woocommerce_after_main_content', 'shareblock_wc_after_main_content', 10 );
add_action( 'woocommerce_before_shop_loop', 'shareblock_wc_before_result_count', 19 );
add_action( 'woocommerce_before_shop_loop', 'shareblock_wc_after_catalog_ordering', 31 );
add_action( 'woocommerce_before_main_content', 'shareblock_remove_single_breadcrumb', 1 );
add_action( 'woocommerce_after_quantity_input_field', 'shareblock_wc_quantity_button' );
add_filter( 'woocommerce_add_to_cart_fragments', 'shareblock_wc_add_to_cart', 10 );
add_filter( 'woocommerce_output_related_products_args', 'shareblock_wc_related_posts_per_page' );
add_filter( 'woocommerce_product_additional_information_heading', 'shareblock_additional_information_heading' );
add_filter( 'woocommerce_get_image_size_single', 'shareblock_wc_image_size_single', 10 );
add_filter( 'loop_shop_per_page', 'shareblock_wc_shop_products_per_page', 99 );
add_filter( 'woocommerce_cross_sells_columns', 'shareblock_wc_cross_sells_columns' );
add_filter( 'woocommerce_sale_flash', 'shareblock_wc_sale_percent', 10, 3 );
add_filter( 'loop_shop_columns', 'shareblock_wc_shop_columns' );
...
if ( ! function_exists( 'shareblock_buttons_wrapper_open' ) ) :
    function shareblock_buttons_wrapper_open() {
        echo '<div class="product-buttons  cart-tooltips">';
    }
endif;


if ( ! function_exists( 'shareblock_buttons_wrapper_close' ) ) :
    function shareblock_buttons_wrapper_close() {
        echo '</div>';
    }
endif;


if ( ! function_exists( 'shareblock_loop_product_title' ) ) :
    function shareblock_loop_product_title() {
        ?>
...
if ( ! function_exists( 'shareblock_loop_product_thumbnail' ) ) :
    function shareblock_loop_product_thumbnail() {
        ?>
...
if ( ! function_exists( 'shareblock_checkout_customer_details_before' ) ):
    function shareblock_checkout_customer_details_before() {
        ?>
...
if ( ! function_exists( 'shareblock_checkout_customer_details_after' ) ):
    function shareblock_checkout_customer_details_after() {
        ?>
...
if ( ! function_exists( 'shareblock_checkout_order_after' ) ):
    function shareblock_checkout_order_after() {
        ?>
        </div>
    <?php
    }
...
if ( ! function_exists( 'shareblock_cart_after' ) ):
    function shareblock_cart_after() {
        ?>
...
if ( ! function_exists( 'shareblock_single_product_before' ) ):
    function shareblock_single_product_before() {
        ?>
...
if ( ! function_exists( 'shareblock_single_product_after' ) ):
    function shareblock_single_product_after() {
        ?>
        </div>
    <?php
    }
...
if ( ! function_exists( 'shareblock_single_product_image_before' ) ):
    function shareblock_single_product_image_before() {
        ?>
...
if ( ! function_exists( 'shareblock_single_product_summary_before' ) ):
function shareblock_single_product_summary_before() {
    ?>
    </div></div>
    <div class="jl-wc-dec">
<?php
...
if ( ! function_exists( 'shareblock_single_product_summary_after' ) ):
    function shareblock_single_product_summary_after() {
        ?>
        </div></div>
    <?php
    }
...
if ( ! function_exists( 'shareblock_additional_information_heading' ) ) :
    function shareblock_additional_information_heading( $heading ) {
        return false;
    }
endif;


if ( ! function_exists( 'shareblock_wc_image_size_single' ) ) :
    function shareblock_wc_image_size_single( $size ) {
...
if ( ! function_exists( 'shareblock_wc_related_posts_per_page' ) ) :
    function shareblock_wc_related_posts_per_page( $args ) {
        $total                  = 4;
...
if ( ! function_exists( 'shareblock_wc_cross_sells_columns' ) ) :
    function shareblock_wc_cross_sells_columns( $columns ) {
        return 4;
    }

endif;

if ( ! function_exists( 'shareblock_wc_before_shop_loop' ) ):
    function shareblock_wc_before_shop_loop() {
        if ( is_shop() ):
        ...
        if ( ! empty( $shareblock_wc_sidebar ) && ! empty( $shareblock_wc_sidebar_position ) ) : ?>
            <div class="page-content jl-content-section is-sidebar-<?php echo esc_attr( $shareblock_wc_sidebar_position ) ?>">
...
if ( ! function_exists( 'shareblock_wc_after_shop_loop' ) ):
    function shareblock_wc_after_shop_loop() {
        echo '</div></div>';
    }
endif;

if ( ! function_exists( 'shareblock_wc_after_main_content' ) ) {
    function shareblock_wc_after_main_content() {
        echo '</div></div>';
    }
}

if ( ! function_exists( 'shareblock_wc_before_result_count' ) ) {
    function    shareblock_wc_before_result_count() {
        echo '<aside class="result-wrap">';
    }
}

if ( ! function_exists( 'shareblock_wc_after_catalog_ordering' ) ) {
    function shareblock_wc_after_catalog_ordering() {
        echo '</aside>';
    }
}

if ( ! function_exists( 'shareblock_wc_shop_columns' ) ) {
    function shareblock_wc_shop_columns() {
        ...
        if ( ! empty( $shareblock_wc_sidebar ) ) {
            return 3;
        } else {
            return 4;
        }
...
if ( ! function_exists( 'shareblock_wc_sale_percent' ) ):
    function shareblock_wc_sale_percent( $html, $post, $product ) {
        $sale_percent = '';
...
if ( ! function_exists( 'shareblock_wc_add_to_cart' ) ) {
    function shareblock_wc_add_to_cart( $fragments ) {
        ob_start(); ?>
...
if ( ! function_exists( 'shareblock_wc_shop_products_per_page' ) ) {
    function shareblock_wc_shop_products_per_page( $total ) {
        $posts_per_page = 12;
...
if ( ! function_exists( 'shareblock_remove_single_breadcrumb' ) ) {
    function shareblock_remove_single_breadcrumb() {
        if ( is_product() ) {
...
if ( ! function_exists( 'shareblock_wc_quantity_button' ) ) {
    function shareblock_wc_quantity_button() {
        ?>
